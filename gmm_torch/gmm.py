...
class GaussianMixture(torch.nn.Module):
        ...
        if self.covariance_type == "diag":
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(
                    self.var_init.to(self.covariance_data_type), requires_grad=False
                )
            else:
                self.var = torch.nn.Parameter(
                    torch.ones(
                        ...
                        self.n_components,
                        self.n_features,
                        dtype=self.covariance_data_type,
                    ),
                    requires_grad=False,
                )
            ...
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(
                    torch.eye(self.n_features, dtype=self.covariance_data_type)
    ...
    def _estimate_log_prob(self, x):
        ...
        if self.covariance_type == "full":
            ...
            def get_required_memory(sub_K):
                ...
                calculate_matmul_requires = (
                    sub_K * N * D + sub_K * N
                ) * x.element_size()
                return (
                    x_mu_requires
                    + x_mu_T_precision_requires
                    + calculate_matmul_requires
                )

            n_splits = find_optimal_splits(
                K, get_required_memory, x.device, safe_mode=self.safe_mode
            )
            sub_K = ceil(K / n_splits)
            for i in range(n_splits):
                K_start = i * sub_K
                K_end = min((i + 1) * sub_K, K)
                sub_x_mu = x - mu[:, K_start:K_end, :]  # [N, sub_K, D]
                sub_x_mu_T_precision = (
                    sub_x_mu.transpose(0, 1) @ precision[:, K_start:K_end]
                ).transpose(0, 2)
                sub_x_mu_T_precision_x_mu = calculate_matmul(
    ...
    def _calculate_log_det(self, var):
        ...
        assert (
            var != var
        ).sum() == 0, "`var` contains NaN, set `covariance_data_type` to double"
        assert (
            var.abs() == float("inf")
        ).sum() == 0, "`var` contains inf, set `covariance_data_type` to double"
